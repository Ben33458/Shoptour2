...
@section('title', 'Kundengruppen importieren (CSV)')
...
@if(isset($result))
    ...
    @endif
    <a href="{{ route('admin.imports.customer-groups') }}" class="btn btn-outline">Neuen Import starten</a>
    <a href="{{ route('admin.customer-groups.index') }}" class="btn btn-primary" style="margin-left:8px">Zu den Kundengruppen</a>
    @php return @endphp
@endif
...
<div class="card" style="margin-bottom:24px">
    ...
    <div class="card-body">
        <p class="text-muted" style="margin-top:0">
            Identifizierung über <code>name</code> (Pflichtfeld).<br>
            Erlaubte Spalten:
            <code>name, active, is_business, is_deposit_exempt,
            price_adjustment_type (none|fixed|percent),
            price_adjustment_percent (z.B. 5.00 für 5&nbsp;%),
            price_adjustment_fixed_eur (z.B. 1.50)</code>
        </p>
        <form method="POST" action="{{ route('admin.imports.customer-groups.upload') }}" enctype="multipart/form-data">
            @csrf
...
@if(isset($preview))
    ...
    <form method="POST" action="{{ route('admin.imports.customer-groups.execute') }}">
        @csrf
        ...
        <div class="flex gap-2 items-center">
            ...
            <a href="{{ route('admin.imports.customer-groups') }}" class="btn btn-outline">Abbrechen</a>
        </div>
    </form>
@endif
