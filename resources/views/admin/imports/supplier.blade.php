...
@section('title', 'Lieferanten importieren (CSV)')
...
@if(isset($result))
    ...
    @endif
    <a href="{{ route('admin.imports.suppliers') }}" class="btn btn-outline">Neuen Import starten</a>
    <a href="{{ route('admin.suppliers.index') }}" class="btn btn-primary" style="margin-left:8px">Zu den Lieferanten</a>
    @php return @endphp
@endif
...
<div class="card" style="margin-bottom:24px">
    ...
    <div class="card-body">
        <p class="text-muted" style="margin-top:0">
            Identifizierung über <code>lieferanten_nr</code> (Pflichtfeld).<br>
            Erlaubte Spalten: <code>lieferanten_nr, name, contact_name, email, phone, address, active</code>
        </p>
        <form method="POST" action="{{ route('admin.imports.suppliers.upload') }}" enctype="multipart/form-data">
            @csrf
...
@if(isset($preview))
    ...
    <form method="POST" action="{{ route('admin.imports.suppliers.execute') }}">
        @csrf
        ...
        <div class="flex gap-2 items-center">
            ...
            <a href="{{ route('admin.imports.suppliers') }}" class="btn btn-outline">Abbrechen</a>
        </div>
    </form>
@endif
