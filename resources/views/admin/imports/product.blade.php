...
@section('title', 'Produkte importieren (CSV)')
...
@if(isset($result))
    ...
    <a href="{{ route('admin.imports.products') }}" class="btn btn-outline">Neuen Import starten</a>
    @php return @endphp
@endif
...
<div class="card" style="margin-bottom:24px">
    ...
    <div class="card-body">
        <p class="text-muted" style="margin-top:0">
            Identifizierung über <code>artikelnummer</code> (Pflichtfeld).<br>
            Erlaubte Spalten: <code>artikelnummer, produktname, base_price_net_milli,
            base_price_net_eur, base_price_gross_milli, base_price_gross_eur, active, ean</code><br>
            Mehrere EANs: mit <code>|</code> trennen (erster = primär).
        </p>
        <form method="POST" action="{{ route('admin.imports.products.upload') }}"
...
@if(isset($preview))
    ...
    <form method="POST" action="{{ route('admin.imports.products.execute') }}">
        @csrf
        ...
        <div class="flex gap-2 items-center">
            ...
            <a href="{{ route('admin.imports.products') }}" class="btn btn-outline">Abbrechen</a>
        </div>
    </form>
